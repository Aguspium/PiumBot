...
class ListaCancionesView(discord.ui.View):
    ...
    async def send_embed(self):
        ...
        canciones_formateadas = [
            f"{i + 1}. {cancion.replace('.opus', '')}" for i, cancion in enumerate(self.canciones[self.index:self.index + self.page_size])
        ]

        
        descripcion = "\n".join(canciones_formateadas)
    ...
    async def next_page(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.index + self.page_size < len(self.canciones):
            ...
            await interaction.response.send_message("Ya estas en la ultima pagina.", ephemeral=True)
    ...
    def get_embed(self):
            ...
            if len(self.canciones) > self.page_size:
                embed.set_footer(text=f"Página {self.index // self.page_size + 1} de {len(self.canciones) // self.page_size + 1}")
